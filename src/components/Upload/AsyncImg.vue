...
<script setup lang="ts" name="AsyncImg">
...
interface UploadFileProps {
  ...
  successCallback: () => void; // 图片上传成功后的回调函数 ==> 必传
...
const handleHttpUpload = async (options: UploadRequestOptions) => {
  if (!props.imageUrl || !props.imageUrl.includes("blob:") || errUpload.value) return;
...
const onChange = (uploadFile: UploadFile) => {
  if (uploadFile?.url?.includes("blob:")) {
    emit("update:imageUrl", uploadFile?.url ?? "");
    formItemContext?.prop && formContext?.validateField([formItemContext.prop as string]);
  }
};
// 图片限制尺寸
const limitImgSize = file => {
  const size = props.sizeWidthHeight![0] / props.sizeWidthHeight![1];
  const _URL = window.URL || window.webkitURL;
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.onload = () => {
      const valid = img.width / img.height === size;
    ...
    () => {
      ElNotification({
        title: "温馨提示",
        message: `上传图片不符合所需的宽高比${props.sizeWidthHeight?.[0]}:${props.sizeWidthHeight?.[1]}！`,
        type: "warning"
      });
      return false;
    }
...
const beforeUpload: UploadProps["beforeUpload"] = async rawFile => {
  ...
  if (!imgType || !imgSize) {
    emit("update:imageUrl", "");
    errUpload.value = true;
  } else if (props.sizeWidthHeight) {
    const limitSize = await limitImgSize(rawFile);
    if (!limitSize) {
      emit("update:imageUrl", "");
      errUpload.value = true;
    }
...
const submitUpload = async () => {
  if (!props.imageUrl.includes("blob:")) return props.successCallback?.();
