<template>
  <div class="upload-box" :style="showLimt()">
    <el-upload
...
<script setup lang="ts" name="AsyncImgs">
...
const showLimt = () => {
  return {
    "--show-upload": props.fileList.length >= props.limit ? "none" : "block"
  };
};
...
const beforeUpload: UploadProps["beforeUpload"] = async rawFile => {
  ...
  if (imgType && imgSize && props.sizeWidthHeight) {
    const limitSize = await limitImgSize(rawFile);
    !limitSize && (errUpload.value = true);
    return limitSize;
  }
  !errUpload.value && (errUpload.value = !imgType || !imgSize);
